...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Verificador de duplicidade - Sequencial")
    ...
    def _deduplicar_worker(self, dir_origem, dir_destino):
        arquivos_processados = []  # lista sequencial de arquivos copiados
        total_arquivos = 0
        arquivos_copiados = 0
        duplicatas = 0
        processados = 0
        ...
        try:
            ...
            for tamanho, caminho_completo in arquivos_completos:
                ...
                # BUSCA SEQUENCIAL
                for tam_existente, arquivo_existente in arquivos_processados:
                    comparacoes += 1
                    if tam_existente == tamanho and comparar_arquivos(caminho_completo, arquivo_existente):
                ...
                if not duplicado:
                    ...
                    arquivos_processados.append((tamanho, destino_final))
                    arquivos_copiados += 1
            ...
            mesmo_tamanho_diferentes = sum(1 for tam, arq in arquivos_processados if [1 for t, a in arquivos_processados if t == tam and a != arq])
            self._log("=" * 60)
            self._log("RESULTADO:")
            ...
            tempo_medio = duracao_total / max(total_arquivos, 1)
